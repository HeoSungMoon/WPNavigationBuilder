...
#include "CustomWorldPartitionNavigationBuilder.h"
...
DEFINE_LOG_CATEGORY_STATIC(LogCustomWorldPartitionNavigationBuilder, Log, All);

UCustomWorldPartitionNavigationBuilder::UCustomWorldPartitionNavigationBuilder(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

bool UCustomWorldPartitionNavigationBuilder::PreRun(UWorld* World, FPackageSourceControlHelper& PackageHelper)
{
	...
	for (const TObjectPtr<UDataLayerAsset>& DataLayer : World->GetWorldSettings()->BaseNavmeshDataLayers)
	{
		if (DataLayer != nullptr)
		{
			...
			{
				UE_LOG(LogCustomWorldPartitionNavigationBuilder, Error, TEXT("Missing UDataLayerInstance for %s."), *DataLayer->GetName());
			}
	...
	UE_LOG(LogCustomWorldPartitionNavigationBuilder, Log, TEXT("Starting NavigationDataBuilder"));
	UE_LOG(LogCustomWorldPartitionNavigationBuilder, Log, TEXT("   ANavigationDataChunkActor GridSize: %8i"), GridSize);
	UE_LOG(LogCustomWorldPartitionNavigationBuilder, Log, TEXT("   IterativeCellSize:                  %8i (%ix%i navigation datachunk partition actor per loaded cell)"), IterativeCellSize, IterativeCellSize/GridSize, IterativeCellSize/GridSize);
	UE_LOG(LogCustomWorldPartitionNavigationBuilder, Log, TEXT("   IterativeCellOverlapSize:           %8i"), IterativeCellOverlapSize);
	
	return true;
}

bool UCustomWorldPartitionNavigationBuilder::RunInternal(UWorld* World, const FCellInfo& InCellInfo, FPackageSourceControlHelper& PackageHelper)
{
	UE_LOG(LogCustomWorldPartitionNavigationBuilder, Verbose, TEXT(" "));
	UE_LOG(LogCustomWorldPartitionNavigationBuilder, Verbose, TEXT("============================================================================================================"));
	UE_LOG(LogCustomWorldPartitionNavigationBuilder, Verbose, TEXT("RunInternal"));
	UE_LOG(LogCustomWorldPartitionNavigationBuilder, Verbose, TEXT("   Bounds %s ."), *InCellInfo.Bounds.ToString());
	...
	if (bCleanBuilderPackages)
	{
		...
		for (TActorIterator<ANavigationDataChunkActor> ItActor(World); ItActor; ++ItActor)
		{
			...
			if (Package)
			{
				...
				{
					UE_LOG(LogCustomWorldPartitionNavigationBuilder, Verbose, TEXT("   Skippping package file already deleted: %s"), *Package->GetName());
				}
			}
		}

		UE_LOG(LogCustomWorldPartitionNavigationBuilder, Verbose, TEXT("   Number of packages to clear: %i"), PackagesToClean.Num());

		// Just delete all ANavigationDataChunkActor packages
		if (!DeletePackages(PackageHelper, PackagesToClean.Array()))
		{
			UE_LOG(LogCustomWorldPartitionNavigationBuilder, Error, TEXT("Error deleting packages."));
		}
	...
	UE_LOG(LogCustomWorldPartitionNavigationBuilder, Verbose, TEXT("   GeneratingBounds %s"), *GeneratingBounds.ToString());
	...
	for (TActorIterator<ANavigationDataChunkActor> It(World); It; ++It)
	{
		...
		UE_LOG(LogCustomWorldPartitionNavigationBuilder, Verbose, TEXT("   Location %s %s (%s %s)"),
		...
		{
			UE_LOG(LogCustomWorldPartitionNavigationBuilder, Verbose, TEXT("   Destroy actor %s in package %s."), *Actor->GetName(), *Actor->GetPackage()->GetName());
			const bool bDestroyed = World->DestroyActor(Actor);
			if (!bDestroyed)
			{
				UE_LOG(LogCustomWorldPartitionNavigationBuilder, Warning, TEXT("      Could not be destroy %s."), *Actor->GetName());
			}
		}
	}
	UE_LOG(LogCustomWorldPartitionNavigationBuilder, Verbose, TEXT("   Number of ANavigationDataChunkActor: %i"), Count);
	...
	for (TActorIterator<ANavigationDataChunkActor> ItActor(World); ItActor; ++ItActor)
	{
		...
		UE_LOG(LogCustomWorldPartitionNavigationBuilder, Verbose, TEXT("   Adding package %s (from actor %s)."), *String, *ItActor->GetName());
	}
	...
	for (UPackage* ActorPackage : NavigationDataChunkActorPackages)
	{
		...
		if (ActorPackage && ActorPackage->IsDirty())
		{
			if (UPackage::IsEmptyPackage(ActorPackage))
			{
				...
				{
					UE_LOG(LogCustomWorldPartitionNavigationBuilder, Verbose, TEXT("   Skippping package file already deleted: %s"), *ActorPackage->GetName());
				}
			}
			else
			{
	...
	{
		{
			...
			UE_LOG(LogCustomWorldPartitionNavigationBuilder, Log, TEXT("Checking out %d packages."), PackagesToAdd.Num());
			...
			else
			{
				...
				for (const UPackage* Package : PackagesToAdd)
				{
					...
					if (IPlatformFile::GetPlatformPhysical().FileExists(*PackageFilename))
					{
						...
						{
							UE_LOG(LogCustomWorldPartitionNavigationBuilder, Error, TEXT("Error setting %s writable"), *PackageFilename);
							return true;
						}
					}
				}
		...
		{
			{
				...
				UE_LOG(LogCustomWorldPartitionNavigationBuilder, Log, TEXT("Adding packages to revision control."));

				if (!PackageHelper.AddToSourceControl(PackagesToAdd))
				{
					UE_LOG(LogCustomWorldPartitionNavigationBuilder, Error, TEXT("Error adding packages."));
					return true;
				}
			}
...
bool UCustomWorldPartitionNavigationBuilder::PostRun(UWorld* World, FPackageSourceControlHelper& PackageHelper, const bool bInRunSuccess)
{
	...
	{
		UE_LOG(LogCustomWorldPartitionNavigationBuilder, Error, TEXT("Error auto-submitting packages."));
		return true;
	}
...
bool UCustomWorldPartitionNavigationBuilder::GenerateNavigationData(UWorldPartition* WorldPartition, const FBox& LoadedBounds, const FBox& GeneratingBounds) const
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UCustomWorldPartitionNavigationBuilder::GenerateNavigationData);

	static int32 CallCount = 0;
	CallCount++;
	UE_LOG(LogCustomWorldPartitionNavigationBuilder, Log, TEXT("Iteration %i. GenerateNavigationData for LoadedBounds %s"), CallCount, *LoadedBounds.ToString());
	...
	{
		UE_LOG(LogCustomWorldPartitionNavigationBuilder, Verbose, TEXT("No navigation system to generate navigation data."));
		return false;
	}
	...
	if (!LoadedBounds.Intersect(NavBounds))
	{
		...
		UE_LOG(LogCustomWorldPartitionNavigationBuilder, Log, TEXT("GenerateNavigationData finished (no intersection with nav bounds)."));
		return true;
	}
	...
	{
		// No intersections, early out
		UE_LOG(LogCustomWorldPartitionNavigationBuilder, Log, TEXT("GenerateNavigationData finished (no intersection with generated navigation data)."));
		return true;
	}
	...
	FActorPartitionGridHelper::ForEachIntersectingCell(NavigationDataActorClass, GeneratingBounds2D, World->PersistentLevel,
		[&WorldPartition, &ActorCount, World, &NavDataBounds, GridSize, this](const UActorPartitionSubsystem::FCellCoord& InCellCoord, const FIntRect& InCellBounds)->bool
		{
			...
			if (CellBounds.GetExtent().X < 1.f || CellBounds.GetExtent().Y < 1.f)
			{
				...
				UE_LOG(LogCustomWorldPartitionNavigationBuilder, Warning, TEXT("%s: grid cell too small."), ANSI_TO_TCHAR(__FUNCTION__));
				return false;
			}
			...
			UE_LOG(LogCustomWorldPartitionNavigationBuilder, VeryVerbose, TEXT("Setting ChunkActorBounds to %s"), *ChunkActorBounds.ToString());
			...
			UE_LOG(LogCustomWorldPartitionNavigationBuilder, Verbose, TEXT("%i) %s added."), ActorCount, *DataChunkActor->GetName());

			return true;
		});

	UE_LOG(LogCustomWorldPartitionNavigationBuilder, Log, TEXT("GenerateNavigationData finished (%i actors added)."), ActorCount);
	return true;
}

bool UCustomWorldPartitionNavigationBuilder::SavePackages(const TArray<UPackage*>& PackagesToSave) const
{
	// Save packages
	TRACE_CPUPROFILER_EVENT_SCOPE(SavingPackages);
	UE_LOG(LogCustomWorldPartitionNavigationBuilder, Log, TEXT("Saving %d packages."), PackagesToSave.Num());

	for (UPackage* Package : PackagesToSave)
	{
		UE_LOG(LogCustomWorldPartitionNavigationBuilder, Verbose, TEXT("   Saving package  %s."), *Package->GetName());
		FString PackageFileName = SourceControlHelpers::PackageFilename(Package);
		...
		{
			UE_LOG(LogCustomWorldPartitionNavigationBuilder, Error, TEXT("   Error saving package %s."), *Package->GetName());
			return false;
		}
	}
...
bool UCustomWorldPartitionNavigationBuilder::DeletePackages(const FPackageSourceControlHelper& PackageHelper, const TArray<UPackage*>& PackagesToDelete) const
{
	if (!PackagesToDelete.IsEmpty())
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(DeletePackages);
		
		UE_LOG(LogCustomWorldPartitionNavigationBuilder, Log, TEXT("Deleting %d packages."), PackagesToDelete.Num());
		for (const UPackage* Package : PackagesToDelete)
		{
			UE_LOG(LogCustomWorldPartitionNavigationBuilder, Verbose, TEXT("   Deleting package  %s."), *Package->GetName());
		}
